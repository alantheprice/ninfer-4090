...
void attn_input_proj(const Tensor& x, const Weight& query_key_weight,
                     ...
                     cudaStream_t stream) {
    // Two admitted geometries: Qwen3.8-27B (hidden 5120, q 6144, kv 1024) and
    // Qwen3.5-9B (hidden 4096, q 4096, kv 1024). Dispatch on x height.
    const std::int32_t kHidden = x.ne[0];
    const std::int32_t kQRows  = kHidden == 5120 ? 6144 : 4096;
