...
using NineBGeometry           = Q4DraftHeadGeometry<4096>;
...
constexpr auto kNineBLaunchers = make_launchers<NineBGeometry, kFirstSmallT>(
    std::make_index_sequence<kLastOptimizedT - kFirstSmallT + 1>{});
...
void launch_q4_draft_head_small_t(const Tensor& x, const Weight& weight, Tensor& out,
    ...
    if (matches<NineBGeometry>(x, weight) && x.ne[1] <= kLastOptimizedT) {
        kNineBLaunchers[static_cast<std::size_t>(x.ne[1] - kFirstSmallT)](x, weight, out, stream);
        return;
    }
