...
public:
    ...
    using Executor9B = runtime::ConcurrentExecutor<targets::Qwen3_5_9BInstance>;
    ...
    using Executor = std::variant<std::monostate, std::unique_ptr<Executor27>,
                                  std::unique_ptr<Executor9B>, std::unique_ptr<Executor35>>;
        ...
        executor          = std::visit(
            [&](auto& target_ptr) -> Executor {
                ...
                if constexpr (std::is_same_v<Instance, targets::Qwen3_6_27BInstance>) {
                    ...
                    return std::make_unique<Executor9B>(*target_ptr, options);
                } else {
